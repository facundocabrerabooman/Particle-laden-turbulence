function video3Dtraj(particle_part,tracer_tracks,kexp,trajlen,trajincrp,trajincrt,Rmin,Rmax)
...
%%
idxp = find(part.T<kexp*Nframemax & part.T>(kexp-1)*Nframemax);
idxt = find(tracer.T<kexp*Nframemax & tracer.T>(kexp-1)*Nframemax);
...
%         surf(part.Xf(idx_trajp(j))+xs,part.Zf(idx_trajp(j))+zs,part.Yf(idx_trajp(j))+ys,colorp);hold on
    ...
    neighborAll(i) = neighborInfos(part,tracer,idx1,idx3,Rmin,Rmax);
...
%     plot3(tracer.Xf(neighbor_global(i).idx),tracer.Zf(neighbor_global(i).idx),tracer.Yf(neighbor_global(i).idx),'ro');
    plot3(tracer.Xf(neighborAll(i).idxfront),tracer.Zf(neighborAll(i).idxfront),tracer.Yf(neighborAll(i).idxfront),'ro');
    plot3(tracer.Xf(neighborAll(i).idxback),tracer.Zf(neighborAll(i).idxback),tracer.Yf(neighborAll(i).idxback),'ko');
