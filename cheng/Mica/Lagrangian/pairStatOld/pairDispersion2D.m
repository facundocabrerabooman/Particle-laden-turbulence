function [X1,X2,Y1,Y2,R2,len,nt]=dispersion(traj,minoverlap,varargin)
...
for nn=1:Ntracks;
    ...
    %for k=1:nframes(j)
        frame0(j)=traj(I1(1),6);
        frame(j,1:nframes(j))=traj(I1,6);
        x(j,1:nframes(j))=traj(I1,1);
        y(j,1:nframes(j))=traj(I1,2);
...
if(j>0)
    ...
    if(~isempty(i_overlap))
        for ii=1:size(i_overlap,1)
            ...
            R2(ii_tot+ii).d2=(X1(ii_tot+ii,:)-X2(ii_tot+ii,:)).^2+(Y1(ii_tot+ii,:)-Y2(ii_tot+ii,:)).^2;
        end
    ...
    %else
    if ~isempty(i_overlap)
        save  dispersion X1 X2 Y1 Y2 R2 len nt ;
    end
