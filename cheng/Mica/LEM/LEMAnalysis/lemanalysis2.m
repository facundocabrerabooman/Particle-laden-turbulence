function [pivf,meanvel,stdvel,mtauwxy,Rv,Sf,Es] = lemanalysis2(name)
...
%%
for k=1:numel(pivf)
    pivf(k).vx=pivf(k).vx-mvx;
    pivf(k).vy=pivf(k).vy-mvy;
end

%% Compute correlation function %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Rv2.Rvxx=corrf(vec2scal(pivf,'ux'),1);
Rv2.Rvyy=corrf(vec2scal(pivf,'uy'),2);
%%
Rv2.Rvxy=corrf(vec2scal(pivf,'ux'),2);
%%
Rv2.Rvyx=corrf(vec2scal(pivf,'uy'),1);
%% graphic corfun
figure; hold on;
...
Sf2.Sxx=ssf(vec2scal(pivf,'ux'),1);
%%
Sf2.Syy=ssf(vec2scal(pivf,'uy'),2);
Sf2.Sxy=ssf(vec2scal(pivf,'ux'),2);
Sf2.Syx=ssf(vec2scal(pivf,'uy'),1); 
%%
figure; hold on;
