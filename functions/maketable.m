...
Name = {'Mean', 'Skewness','Kurtosis'};
vx = [pdfV(1).mean, pdfV(1).skewness, pdfV(1).flatness];
vy = [pdfV(2).mean, pdfV(2).skewness, pdfV(2).flatness];
vz = [pdfV(3).mean, pdfV(3).skewness, pdfV(3).flatness];

ax = [pdfA(1).mean, pdfA(1).skewness, pdfA(1).flatness];
ay = [pdfA(2).mean, pdfA(2).skewness, pdfA(2).flatness];
az = [pdfA(3).mean, pdfA(3).skewness, pdfA(3).flatness];
