function tracksout = calcVelLEM(tracksin,w,l,Fs,w_acc, l_acc)
%%
if exist('w','var')
    filterwidth = w;
    filterlength = l;

    filterwidth_acc = w_acc;
    filterlength_acc = l_acc;
...
kera = accfiltcoef(filterwidth_acc,filterlength_acc);
...
if isfield(tracksout,'t')
    ...
    tracksout = addStructFun(tracksout,'t','Tf_acc',@(X)(X(1+ceil((filterlength_acc-1)/2):end-ceil((filterlength_acc-1)/2))));
end
if isfield(tracksout,'Ntrack')
    ...
    tracksout = addStructFun(tracksout,'Ntrack','Ntrack_acc',@(X)(X(1+ceil((filterlength_acc-1)/2):end-ceil((filterlength_acc-1)/2))));
end
end
