...
% input
fpath0 = 'D:\Facu\Images\sept6a\6sept_a';
...
for i = 1:1
    if numel(dfolders)~=0
        ...
        subfolders = fpath;
        preproc_dirt = [fpath0 '\preproc\preproc_tracer\'];
        preproc_dirp = [fpath0 '\preproc\preproc_particle\'];
    end
%     mkdir(preproc_dirt)
%     mkdir(preproc_dirp)
...
%         imwrite(uint16(Im1t),[preproc_dirt '\' fnameo]);
...
%         imwrite(uint16(Im2t),[preproc_dirt '\' fnameo]);
...
%         imwrite(uint16(Im3t),[preproc_dirt '\' fnameo]);
    end
end
